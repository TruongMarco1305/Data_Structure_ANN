...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
    //YOUR CODE IS HERE
    if(this->m_trainable) this->m_aCached_X = X;
    xt::xarray<double> Y = xt::linalg::tensordot(X,xt::transpose(this->m_aWeights),1);
    if(this->m_bUse_Bias) Y += this->m_aBias;
    return Y;
}

// xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
//     //YOUR CODE IS HERE
//     // If in training mode, cache the input X for use in backward pass
//     if (m_trainable) {
//         m_aCached_X = X;
//     }
//     // // Compute Y = X * W^T + b
//     // // W^T is the transpose of m_aWeights (shape: N_in x N_out)
//     xt::xarray<double> W_T = xt::transpose(m_aWeights); // Transpose weights
//     xt::xarray<double> Y = xt::linalg::dot(X, W_T);     // X: (batch_size x N_in), W_T: (N_in x N_out)
//     // xt::xarray<double> output = xt::linalg::tensordot(X, m_aGrad_W, {1}, {0});
//     if (m_bUse_Bias) {
//         // Add bias to each output sample (broadcast over batch dimension)
//         Y += m_aBias;
//     }
//     return Y;
// }
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    ...
    xt::xarray<double> grad_W = xt::linalg::dot(xt::transpose(DY), m_aCached_X);
    m_aGrad_W += grad_W;
