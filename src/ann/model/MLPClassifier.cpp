...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        //YOUR CODE IS HERE
        double_tensor temp = this->forward(batch.getData());
        if(batch_idx == 1){
            results = temp;
        } else {
            results = xt::concatenate(xt::xtuple(results,temp));
        }
        nsamples += temp.shape(0);
        batch_idx ++;
    }
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for(auto batch: *pLoader){
        // Get data and labels from the batch
        double_tensor X_batch = batch.getData();
        double_tensor t_batch = batch.getLabel();
        
        // Forward pass on the batch
        double_tensor Y_batch = this->forward(X_batch);
        
        // Compute predictions
        double_tensor y_pred = xt::argmax(Y_batch, -1);
        
        // Compute true labels
        double_tensor y_true = t_batch;
        // If labels are one-hot encoded, convert them to class indices
        if (t_batch.dimension() > 1 && t_batch.shape()[1] > 1) {
            y_true = xt::argmax(t_batch, -1);
        }
...
void MLPClassifier::backward(){
    //YOUR CODE IS HERE
    // Compute DY by calling the loss layer's backward method
    double_tensor DY = m_pLossLayer->backward();

    // Traverse all layers in reverse order
