...
V xMap<K,V>::put(K key, V value){
    ...
    V retValue = value;
    //YOUR CODE IS HERE    
    DLinkedList<Entry *> &listAtIndex = this->table[index];
    for (auto entry : listAtIndex)
    {
...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    ...
    for (auto* entry : listAtIndex)
    {
        ...
        listAtIndex.removeItem(entry,xMap<K,V>::deleteEntry);
        this->count --;
        return retValue;
        }
    }
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
    ...
    for (int index = 0; index < this->capacity; index++)
    {
        ...
        {
            if (deleteKeyInMap && deleteValueInMap)
            {
            deleteKeyInMap(entry->key);
...
DLinkedList<K> xMap<K,V>::keys(){
    //YOUR CODE IS HERE
    if(this->count == 0) return DLinkedList<K>();
...
DLinkedList<V> xMap<K,V>::values(){
    //YOUR CODE IS HERE
    if(this->count == 0) return DLinkedList<V>();
